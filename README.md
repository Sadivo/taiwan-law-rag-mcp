...
系統支援帶有上下文記憶的多輪對話，能自動解析指代關係（「那第二條呢？」）。

### 啟用 Query Rewriting（選填）
...
啟用後，系統會在送入 retrieval 前自動執行：語言偵測 → 翻譯（英文查詢）→ 意圖分類 → 查詢改寫 → 上下文擴展。

### Session API 使用方式（PowerShell）

```powershell
# 1. 建立 Session
$resp = Invoke-RestMethod -Method POST -Uri http://localhost:8073/session
$sid = $resp.session_id

# 2. 多輪對話
Invoke-RestMethod -Method POST -Uri "http://localhost:8073/session/$sid/chat" `
  -ContentType "application/json" `
  -Body '{"question": "勞基法加班費怎麼算", "top_k": 5}'
...
# 3. 刪除 Session
Invoke-RestMethod -Method DELETE -Uri "http://localhost:8073/session/$sid"
```

### 意圖分類

`QueryClassifier` 支援五種意圖，系統會依此選擇最適合的 retrieval 策略：

| 意圖 | 觸發條件 | 範例 |
|---|---|---|
| `exact` | 含條號（第 X 條） | 「勞基法第38條」 |
| `comparison` | 含比較關鍵字 | 「比較勞基法和民法的差異」 |
| `definition` | 含定義關鍵字 | 「什麼是勞動契約」 |
| `procedure` | 含程序關鍵字 | 「加班費怎麼算」 |
| `semantic` | 其他情境描述 | 「員工被資遣的權益」 |
...
# 只跑 Query Understanding 相關測試（含 Property-Based Testing）
